...
void test1a(void) {
	...
	GET_TIME_OF_DAY(&time1);						//start from 0. Return maybe 31

	SLEEP(SleepTime);

	GET_TIME_OF_DAY(&time2);						// maybe return 152
...
typedef struct {
	INT16 remember_page_touched[LOOP_COUNT];
} REMEMBER_MEMORY_TOUCHED_RECORD;


void test2f(void) {
	MEMORY_TOUCHED_RECORD *mtr;
	REMEMBER_MEMORY_TOUCHED_RECORD *mmtr;

	short Iterations, Index, Loops;

	mtr = (MEMORY_TOUCHED_RECORD *) calloc(1, sizeof(MEMORY_TOUCHED_RECORD));

	mmtr = (REMEMBER_MEMORY_TOUCHED_RECORD *) calloc(1, sizeof(REMEMBER_MEMORY_TOUCHED_RECORD));
	...
	for (Iterations = 0; Iterations < NUMBER_OF_ITERATIONS; Iterations++) {
		...
		for (Loops = 0; Loops < LOOP_COUNT; Loops++) {
			...
			mtr->page_touched[Z502_REG7]++;		//hdduong
			mmtr->remember_page_touched[(short) Loops] = Z502_REG7;
		}   // End of for Loops
		...
		for (Loops = 0; Loops < LOOP_COUNT; Loops++) {
		//for (Loops = 0; Loops < LOGICAL_PAGES_TO_TOUCH; Loops++) { //hdduong
			//<hdduong
			if (mtr->page_touched[Z502_REG7] == 0) {
				Z502_REG7++;
				continue;
			}
			//>hdduong
			...
			Z502_REG6 = mmtr->remember_page_touched[(short) Loops]; //hdduong
			...
			if (Z502_REG2 != Z502_REG1) {
				...
				//debug
				printf("2. Error with PID= %ld  address= %ld   written= %ld   read= %ld\n",Z502_REG4, Z502_REG3, Z502_REG1, Z502_REG2);
				//debug
			}
		}   // End of for Loops
