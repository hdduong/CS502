...
INT32						frame_table_lock;
INT32						shadow_table_lock;
...
BOOL						transfer_from_fault = FALSE;
...
void	LockFrameTable(INT32 *frame_table_lock);
...
void    fault_handler( void )
 ...
 //
	...
	case INVALID_MEMORY:
		...
		if (shadowFrame == -1)  {			// I never touch before
...
void	LockFrameTable  (INT32 *frame_table_lock) {
	READ_MODIFY( MEMORY_INTERLOCK_BASE + FRAME_TABLE_LOCK, DO_LOCK, SUSPEND_UNTIL_LOCKED, frame_table_lock );
}
void	UnLockFrameTable  (INT32 *frame_table_lock) {
	READ_MODIFY( MEMORY_INTERLOCK_BASE + FRAME_TABLE_LOCK, DO_UNLOCK, SUSPEND_UNTIL_LOCKED, frame_table_lock );
}

void	LockShadowTable  (INT32 *shadow_table_lock) {
	READ_MODIFY( MEMORY_INTERLOCK_BASE + SHADOW_TABLE_LOCK, DO_LOCK, SUSPEND_UNTIL_LOCKED, shadow_table_lock );
}
void	UnLockShadowTable  (INT32 *shadow_table_lock) {
	READ_MODIFY( MEMORY_INTERLOCK_BASE + SHADOW_TABLE_LOCK, DO_UNLOCK, SUSPEND_UNTIL_LOCKED, shadow_table_lock );
}
...
void mem_to_disk(INT32 frame, INT32	logicalPageRequest, INT32 victim_pid) {
	...
	newShadowEntry->process_id = victim_pid;//PCB_Current_Running_Process->process_id;//frameTable->process_id;
...
void get_free_disk_sector(INT16	*diskPtr, INT16 *sectorPtr) {
	...
	else if ((strcmp(PCB_Current_Running_Process->process_name,"test2g_b") == 0) ) {
		for (disk = 3; disk <= MAX_NUMBER_OF_DISKS;disk++) {
			for (sector =1; sector <= MAX_NUMBER_OF_SECTORS; sector++) {
				if (MAP_DISK_SECTOR[disk][sector] == 0) {
					MAP_DISK_SECTOR[disk][sector] = 1;
					*diskPtr  = disk;
	...
	else if ((strcmp(PCB_Current_Running_Process->process_name,"test2g_c") == 0) ) {
		for (disk = 5; disk <= MAX_NUMBER_OF_DISKS;disk++) {
			for (sector =1; sector <= MAX_NUMBER_OF_SECTORS; sector++) {
				if (MAP_DISK_SECTOR[disk][sector] == 0) {
					MAP_DISK_SECTOR[disk][sector] = 1;
					*diskPtr  = disk;
	...
	else if ((strcmp(PCB_Current_Running_Process->process_name,"test2g_d") == 0) ) {
		for (disk = 7; disk <= MAX_NUMBER_OF_DISKS;disk++) {
			for (sector =1; sector <= MAX_NUMBER_OF_SECTORS; sector++) {
				if (MAP_DISK_SECTOR[disk][sector] == 0) {
					MAP_DISK_SECTOR[disk][sector] = 1;
					*diskPtr  = disk;
	...
	else if ((strcmp(PCB_Current_Running_Process->process_name,"test2g_e") == 0) ) {
		for (disk = 9; disk <= MAX_NUMBER_OF_DISKS;disk++) {
			for (sector =1; sector <= MAX_NUMBER_OF_SECTORS; sector++) {
				if (MAP_DISK_SECTOR[disk][sector] == 0) {
					MAP_DISK_SECTOR[disk][sector] = 1;
					*diskPtr  = disk;
